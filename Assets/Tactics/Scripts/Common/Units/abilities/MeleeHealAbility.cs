...
{
    /// <summary>
    /// Unity component for melee heal ability. Range: 1 tile (adjacent friendly units only).
    /// </summary>
    public class MeleeHealAbility : Ability
    {
        private MeleeHealAbilityImpl _impl;

        public override void Initialize(IGridController gridController)
        {
            base.Initialize(gridController);
            _impl = new MeleeHealAbilityImpl(UnitReference);
