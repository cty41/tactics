...
export const AuthoringKindSchema = z.enum([
  "run-map", "event", "treasure", "encounter", "battle-layout", "ai", "skill", "presentation"
]);
...
const SkillExecutionProfileSchema = z.object({ areaRadius: z.number().int().nonnegative(), orderedTargetCount: z.number().int().nonnegative(), summonDefinitionId: z.string().min(1).nullable(), summonCount: z.number().int().nonnegative(), summonLimit: z.number().int().nonnegative(), summonCategory: z.string(), requiresCorpse: z.boolean(), ignoreLineOfSight: z.boolean(), shieldMultiplier: z.number().int().nonnegative(), shieldAbsorbsAllDamage: z.boolean(), cleanseHarmful: z.boolean(), secondaryDamage: z.number().int().nonnegative(), areaShape: z.string(), statusChancePercent: z.number().int().min(0).max(100), detonateStatusContentId: z.string().min(1).nullable(), bounceRange: z.number().int().nonnegative(), bounceCount: z.number().int().nonnegative(), pierceAll: z.boolean(), allowsEmptyTarget: z.boolean(), movementDamagePerCell: z.number().int().nonnegative(), summonAttackContentId: z.string().min(1).nullable(), corruptionCost: z.number().int().nonnegative(), damageScaling: z.string().min(1) }).strict();
...
const DocumentSchemas: Record<AuthoringKind, z.ZodTypeAny> = {
  ...
  "battle-layout": ContentIdDocument.extend({ partySpawns: z.array(CellSchema).min(1), enemySpawns: z.array(CellSchema).min(1), blockedCells: z.array(CellSchema) }),
...
export const AuthoringAssetBatchSpecSchema = z.object({
  schemaVersion: z.literal(1), assets: z.array(AuthoringAssetSpecSchema).min(1)
}).strict();
