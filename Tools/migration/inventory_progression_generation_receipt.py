...
def main() -> None:
    ...
    generated_names={
        ...
        for item in draft_data['definitions'] if item['growthVisible'] and item['contentId'] not in {
    ...
    generated_names.update(''.join(part[:1].upper()+part[1:] for part in item['contentId'][6:].replace('.','-').split('-')) for item in draft_data['internalSkillDependencies'])
    generated_names.update({'SummonBasicMeleeAi','SummonFireDemonAi'})
    owned=[path for path in paths if path.name=="InventoryProgressionCatalog.tres" or path.stem in generated_names]
