...
def compile_evidence(draft:dict,scene:Path,balance:Path|None=None,enemy_speed:Path|None=None)->tuple[dict,dict]:
 ...
 if enemy_speed is not None:
  speed_text=enemy_speed.read_text(encoding='utf-8')
  required_speed_tokens=('script_class="PlayableEnemySpeedProfileResource"','UnitContentIds = PackedStringArray(','Speeds = PackedFloat32Array(')
  if any(token not in speed_text for token in required_speed_tokens):raise ValueError('Playable enemy speed contract is invalid')
  speed_digest='sha256:'+hashlib.sha256(enemy_speed.read_bytes()).hexdigest();artifacts.append({'resourcePath':'res://content/ui/PlayableEnemySpeedProfile.tres','targetHash':speed_digest})
 state={'schemaVersion':1,'batchId':draft['batchId'],'source':draft['source'],'artifacts':artifacts}
 receipt={'schemaVersion':1,'batchId':draft['batchId'],'state':'Validated','ownership':'UnityOwned','canvas':{'width':1600,'height':900,'stretch':'canvas_items+keep'},'canonicalCatalogEntries':74,'sceneHash':digest,'playableBalanceContract':'godot-playable-lv1-balance-v1','playableBalanceHash':balance_digest,'enemySpeedContract':'godot-playable-enemy-speed-v1' if enemy_speed is not None else None,'enemySpeedHash':speed_digest,'idempotency':{'resourceSaverRuns':2,'byteIdentical':True},'visualPayload':'existing_migrated_unit_visuals_only','manualUiInputAcceptance':'passed'}
 return state,receipt

def main()->int:
 p=argparse.ArgumentParser();p.add_argument('--draft',type=Path,required=True);p.add_argument('--scene',type=Path,required=True);p.add_argument('--balance',type=Path);p.add_argument('--enemy-speed',type=Path);p.add_argument('--state',type=Path,required=True);p.add_argument('--receipt',type=Path,required=True);a=p.parse_args();state,receipt=compile_evidence(load_json(a.draft),a.scene,a.balance,a.enemy_speed)
