...
public static class AiEncounterBatchValidator
{
    ...
    {
        batch.Validate();global.Validate();if(batch.Entries.Length!=15||global.Entries.Length is not (74 or 101))throw new InvalidOperationException("AI/Encounter or canonical Catalog count is invalid.");
        ...
        if(skills!=4||ai!=6||layouts!=2||encounters!=3)throw new InvalidOperationException("AI/Encounter Resource category count is invalid.");return new(15,74,skills,ai,layouts,encounters);
    }
}
