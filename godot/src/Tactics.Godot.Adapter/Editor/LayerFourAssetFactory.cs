...
public static class LayerFourAssetFactory
{
    ...
    public static void Build()
    {
        ...
        // The legacy batch still owns its frozen map evidence, while the ownership-closure batch owns the
        // authoritative editable map. Re-running this exporter must not replace that canonical catalog entry.
        bool hasAuthoritativeMap=old.Entries.Any(e=>e.ContentIdValue=="treasure.pure-run.standard-v1");
        GodotResourceEntry[] catalogEntries=entries.Where(e=>!hasAuthoritativeMap||e.ResourceTypeIdValue!="run-map").ToArray();
        var all=old.Entries.Where(e=>catalogEntries.All(n=>n.ContentIdValue!=e.ContentIdValue)).Select(Copy).Concat(catalogEntries).OrderBy(e=>e.ContentIdValue,StringComparer.Ordinal).ToArray();if(all.Length is not (108 or 109 or 114 or 115 or 116 or 119 or 131 or 141 or 142 or 143))throw new InvalidOperationException($"Unsupported canonical Catalog count: {all.Length}");var catalog=new GodotResourceCatalog{Entries=all};Save(catalog,Global);catalog.Validate();
