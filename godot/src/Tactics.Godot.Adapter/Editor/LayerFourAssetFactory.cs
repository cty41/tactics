...
public static class LayerFourAssetFactory
{
    ...
    public static void Build()
    {
        ...
        var all=old.Entries.Where(e=>catalogEntries.All(n=>n.ContentIdValue!=e.ContentIdValue)).Select(Copy).Concat(catalogEntries).OrderBy(e=>e.ContentIdValue,StringComparer.Ordinal).ToArray();if(all.Length is not (108 or 109 or 114 or 115 or 116 or 119 or 131 or 141 or 142))throw new InvalidOperationException($"Unsupported canonical Catalog count: {all.Length}");var catalog=new GodotResourceCatalog{Entries=all};Save(catalog,Global);catalog.Validate();
