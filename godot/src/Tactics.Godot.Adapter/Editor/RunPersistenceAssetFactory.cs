...
public static class RunPersistenceAssetFactory
{
    ...
    public static void Build()
    {
        ...
        var resource=new PureRunDefinitionResource{ContentIdValue=id,EncounterContentIds=encounters,CharacterIds=people.Select(v=>v.GetProperty("characterId").GetString()!).ToArray(),UnitContentIds=people.Select(v=>v.GetProperty("unitContentId").GetString()!).ToArray(),StartingSkillContentIds=people.Select(v=>v.GetProperty("startingSkillContentId").GetString()!).ToArray()};resource.ToCoreDefinition();Save(resource,DefinitionPath);
