...
public static class RunPersistenceAssetFactory
{
    ...
    public static void Build()
    {
        ...
        var resource=new PureRunDefinitionResource{ContentIdValue=id,EncounterContentIds=encounters,CharacterIds=people.Select(v=>v.GetProperty("characterId").GetString()!).ToArray(),UnitContentIds=people.Select(v=>v.GetProperty("unitContentId").GetString()!).ToArray(),StartingSkillContentIds=people.Select(v=>v.GetProperty("startingSkillContentId").GetString()!).ToArray(),StartingSkillChoiceContentIds=new[]{"skill.mage.fireball.lv1","skill.mage.ice-bolt.lv1","skill.mage.lightning.lv1","skill.necromancer.summon-skeleton.lv1","skill.necromancer.amplify-damage.lv1","skill.necromancer.bone-spear.lv1","skill.amazon.thrust.lv1","skill.amazon.poison-spear.lv1","skill.amazon.combat-techniques.lv1"},LayerFourMapContentId="run-map.pure-run.layer4-v1"};resource.ToCoreDefinition();Save(resource,DefinitionPath);
