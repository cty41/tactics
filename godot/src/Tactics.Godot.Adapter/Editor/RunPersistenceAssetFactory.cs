...
public static class RunPersistenceAssetFactory
{
    ...
    public static void Build()
    {
        ...
        var resource=new PureRunDefinitionResource{ContentIdValue=id,EncounterContentIds=encounters,CharacterIds=people.Select(v=>v.GetProperty("characterId").GetString()!).ToArray(),UnitContentIds=people.Select(v=>v.GetProperty("unitContentId").GetString()!).ToArray(),StartingSkillContentIds=people.Select(v=>v.GetProperty("startingSkillContentId").GetString()!).ToArray(),StartingSkillChoiceContentIds=new[]{"skill.mage.fireball.lv1","skill.mage.ice-bolt.lv1","skill.mage.lightning.lv1","skill.necromancer.summon-skeleton.lv1","skill.necromancer.amplify-damage.lv1","skill.necromancer.bone-spear.lv1","skill.amazon.thrust.lv1","skill.poison-spear.lv1","skill.amazon.combat-techniques.lv1"},LayerFourMapContentId="run-map.pure-run.layer4-v1"};resource.ToCoreDefinition();Save(resource,DefinitionPath);
        string[] refs=encounters.Concat(resource.UnitContentIds).Concat(resource.StartingSkillContentIds).Concat(resource.StartingSkillChoiceContentIds).Distinct(StringComparer.Ordinal).Order(StringComparer.Ordinal).ToArray();var entry=Entry(id,"run",DefinitionPath,refs);var batch=new GodotResourceCatalog{Entries=new[]{entry}};Save(batch,CatalogPath);batch.Validate();
        GodotResourceCatalog previous=ResourceLoader.Load<GodotResourceCatalog>(GlobalCatalogPath,string.Empty,ResourceLoader.CacheMode.Ignore)??throw new InvalidOperationException("Canonical Catalog is missing.");var entries=previous.Entries.Where(v=>v.ContentIdValue!=id).Select(Copy).ToList();entries.Add(Copy(entry));var global=new GodotResourceCatalog{Entries=entries.OrderBy(v=>v.ContentIdValue,StringComparer.Ordinal).ToArray()};if(global.Entries.Length is not (74 or 101 or 108 or 114 or 115 or 116 or 119 or 123 or 124 or 125 or 131 or 141 or 142 or 143))throw new InvalidOperationException($"Canonical Catalog count is invalid: {global.Entries.Length}.");Save(global,GlobalCatalogPath);global.Validate();
