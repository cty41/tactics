...
public partial class PresentationProfileWorkbench : VBoxContainer, IAuthoringWorkspaceParticipant
{
    ...
    public override void _Ready()
    {
        ...
        var toolbar = WorkbenchUi.Toolbar(this); toolbar.AddChild(new Label { Text = "PRESENTATION WORKBENCH" }); _picker = new OptionButton { CustomMinimumSize = new Vector2(320, 0) }; _picker.ItemSelected += LoadSelected; toolbar.AddChild(_picker); AddButton(toolbar, "Import", ImportDraft); AddButton(toolbar, "Validate", ValidateDraft); AddButton(toolbar, "Revert", RevertAll); AddChild(toolbar);
        var threePane = new HSplitContainer { SizeFlagsHorizontal = SizeFlags.ExpandFill, SizeFlagsVertical = SizeFlags.ExpandFill };
        _graph = new GraphEdit { SizeFlagsHorizontal = SizeFlags.ExpandFill, SizeFlagsVertical = SizeFlags.ExpandFill, CustomMinimumSize = new Vector2(380, 360) }; WorkbenchUi.StyleGraph(_graph); threePane.AddChild(_graph);
        var previewPane = WorkbenchUi.Pane(this, 460);
        var previewToolbar = WorkbenchUi.Toolbar(this); AddButton(previewToolbar, "Preview", PreviewDraft); AddButton(previewToolbar, "Play", PlayPreview); AddButton(previewToolbar, "Pause", PausePreview); AddButton(previewToolbar, "Stop", StopPreview); previewToolbar.AddChild(new Label { Text = "Scope" }); _previewScope = new OptionButton(); foreach (string value in new[] { "Full", "Action", "Impact" }) _previewScope.AddItem(value); previewToolbar.AddChild(_previewScope); previewToolbar.AddChild(new Label { Text = "Speed" }); _previewSpeed = new OptionButton(); foreach (string value in new[] { "0.5", "1", "2", "4" }) _previewSpeed.AddItem(value); _previewSpeed.Select(1); previewToolbar.AddChild(_previewSpeed); previewPane.AddChild(previewToolbar);
        ...
        _previewStage = new PresentationProfilePreviewStage(); viewport.AddChild(_previewStage); previewFrame.AddChild(viewport); previewPane.AddChild(previewFrame);
        _timelineStatus = new Label { Text = "TIMELINE  • compile or play to inspect duration, activity and cleanup", AutowrapMode = TextServer.AutowrapMode.WordSmart }; previewPane.AddChild(_timelineStatus); threePane.AddChild(previewPane);
        ...
        var graphTools = WorkbenchUi.InspectorSection(this, "Graph & leaf inspector", new Color("9b7cff")); _nodePicker = new OptionButton(); _nodePicker.ItemSelected += SelectNode; graphTools.AddChild(_nodePicker); var nodeActions = new HBoxContainer(); AddButton(nodeActions, "Add Marker", AddMarker); AddButton(nodeActions, "Add Delay", AddDelay); AddButton(nodeActions, "Add Parallel", AddParallel); graphTools.AddChild(nodeActions); var stateActions = new HBoxContainer(); AddButton(stateActions, "Delete", DeleteNode); AddButton(stateActions, "Toggle Enabled", ToggleEnabled); AddButton(stateActions, "Auto Layout", AutoLayout); graphTools.AddChild(stateActions); _targetPicker = new OptionButton(); graphTools.AddChild(_targetPicker); var edgeActions = new HBoxContainer(); AddButton(edgeActions, "Connect", AddEdge); AddButton(edgeActions, "Disconnect", RemoveEdge); graphTools.AddChild(edgeActions); AddButton(graphTools, "Duplicate & Rebind", DuplicateAndRebind); _propertyValue = new LineEdit { PlaceholderText = "selected leaf value" }; graphTools.AddChild(_propertyValue); AddButton(graphTools, "Update Leaf", UpdateLeaf); inspector.AddChild(graphTools);
        var diagnostics = WorkbenchUi.InspectorSection(this, "Canonical snapshot", new Color("9aa4b2"), collapsed: true); var showSnapshot = new CheckButton { Text = "Show raw snapshot" }; diagnostics.AddChild(showSnapshot); _snapshot = new TextEdit { Visible = false, CustomMinimumSize = new Vector2(0, 220), SizeFlagsHorizontal = SizeFlags.ExpandFill }; showSnapshot.Toggled += visible => _snapshot.Visible = visible; diagnostics.AddChild(_snapshot); inspector.AddChild(diagnostics); inspectorScroll.AddChild(inspector); threePane.AddChild(inspectorScroll); AddChild(threePane);
    ...
    public override void _ExitTree() { _previewStage?.Stop(); _dragHandlers.Clear(); _loadAttempts = 0; _resource = null; _draft = null; _graphDraft = null; _previewStage = null; }
    ...
    private bool TryCompilePreview(bool play)
    {
        try
        {
            ...
            _timelineStatus!.Text = $"TIMELINE  {timeline:0.###}s  • nodes {_graphDraft.Nodes.Count}  • shared leaves {shared}  • scope {_previewScope!.GetItemText(_previewScope.Selected)}";
