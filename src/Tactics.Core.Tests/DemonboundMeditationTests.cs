...
public sealed class DemonboundMeditationTests
{
    ...
    [Test]
    public void BaneLevelTwoBuffsBasicMeleeAndAppliesDamageDebuff()
    {
        ...
        var bane = new SkillDefinition(new ContentId("skill.demonbound.bane.lv2"), "godot.bane",
            SkillRole.Demonbound, SkillKind.Active, 2, 3, 0, 0, SkillExecutionKind.Bane, 0,
        ...
        BattleTransition enchanted = new BattleTransitionService().Apply(state,
            new UseSkillCommand(state.ActiveUnitId, state.ActiveUnitId,
                state.Units[state.ActiveUnitId].Unit.Position, bane));
        BattleUnitState enemy = enchanted.State.Units[new UnitInstanceId("enemy.0")];
        ...
        BattleTransition hit = new BattleTransitionService().Apply(enchanted.State,
            new UseSkillCommand(state.ActiveUnitId, enemy.Unit.InstanceId, enemy.Unit.Position, melee));

        Assert.Multiple(() =>
        {
            Assert.That(enchanted.State.Units[state.ActiveUnitId].CurrentMana, Is.EqualTo(3));
            Assert.That(enchanted.State.Units[state.ActiveUnitId].DemonboundState!.Corruption, Is.EqualTo(1));
            Assert.That(hit.Events.OfType<DamageAppliedEvent>().Single().Amount, Is.EqualTo(5));
            Assert.That(hit.State.Units[enemy.Unit.InstanceId].Statuses.Values.Any(status =>
                status.EffectKind == Tactics.Core.Statuses.StatusEffectKind.DamageOutputReduction), Is.True);
        });
    }
    ...
    private static BattleState CreateState(DemonboundBattleState demonbound)
    {
        ...
        return new BattleState(new BoardSnapshot(cells), new[]
        {
            ...
            new BattleUnitState(new UnitState(enemyId, new ContentId("unit.enemy"), new GridPoint(2, 1),
                3, 8, 1, 1), 20, 20)
        }, new[] { heroId, enemyId }, randomState: 42);
    }
}
