...
public sealed record AiDefinitionDraft(string ContentId,string Archetype,float DistanceWeight,float DamageWeight,float TargetCountWeight,float HarmfulStatusWeight,IReadOnlyList<string> SkillIds,IReadOnlyList<string> PatternSkillIds,AiDecisionGraphDefinition? DecisionGraph=null,int MaximumEngageCandidatesPerTarget=3,int PreferredMinimumRange=1,int PreferredMaximumRange=1,float PreferredRangeRepositionBonus=0);
public sealed record BattleLayoutDraft(string ContentId,IReadOnlyList<GridPoint> PartySpawns,IReadOnlyList<GridPoint> EnemySpawns,IReadOnlyList<GridPoint> BlockedCells,IReadOnlyList<GridPoint>? ShallowWaterCells=null);
...
public sealed class AiEncounterDefinitionCompiler
{
    public AiEncounterCompileResult Compile(IEnumerable<AiDefinitionDraft> aiDrafts,IEnumerable<BattleLayoutDraft> layoutDrafts,IEnumerable<EncounterDefinitionDraft> encounterDrafts)
    {
        ...
        try
        {
            foreach(AiDefinitionDraft draft in aiDrafts){var id=new ContentId(draft.ContentId); if(!Enum.TryParse(draft.Archetype,true,out AiArchetype archetype)||!Enum.IsDefined(archetype))throw new ArgumentException($"Unknown AI archetype '{draft.Archetype}'."); var skills=draft.SkillIds.Select(value=>new ContentId(value)).ToArray(); var pattern=draft.PatternSkillIds.Select(value=>new ContentId(value)).ToArray(); if(draft.MaximumEngageCandidatesPerTarget<1||draft.PreferredMinimumRange<1||draft.PreferredMaximumRange<draft.PreferredMinimumRange)throw new ArgumentException("AI movement contract is invalid."); if(!ai.TryAdd(id,new AiDefinition(id,archetype,new AiProfileDefinition(draft.DistanceWeight,draft.DamageWeight,draft.TargetCountWeight,draft.HarmfulStatusWeight),skills,pattern,draft.DecisionGraph,draft.MaximumEngageCandidatesPerTarget,draft.PreferredMinimumRange,draft.PreferredMaximumRange,draft.PreferredRangeRepositionBonus)))throw new ArgumentException($"Duplicate AI '{id}'."); content.Add(new ContentDraft(id,"ai",1,skills));}
            foreach(BattleLayoutDraft draft in layoutDrafts){var id=new ContentId(draft.ContentId); var value=new BattleLayoutDefinition(id,draft.PartySpawns,draft.EnemySpawns,draft.BlockedCells,draft.ShallowWaterCells); _=new EncounterResolver().Resolve(new EncounterDefinition(new ContentId("encounter.validation"),id,Array.Empty<EncounterMonsterDefinition>()),value); if(!layouts.TryAdd(id,value))throw new ArgumentException($"Duplicate layout '{id}'."); content.Add(new ContentDraft(id,"battle-layout",draft.ShallowWaterCells is { Count: >0 }?2:1));}
