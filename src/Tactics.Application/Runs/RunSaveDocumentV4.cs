...
public sealed record RunSaveDecodeResultV4(bool Succeeded, string? ErrorCode, PureRunSaveSnapshot? Snapshot, int MigratedFromSchema);

/// <summary>V4 persists the complete seven-layer map, late battle checkpoints, and boss summary.</summary>
public static class RunSaveDocumentV4
{
    public const int SchemaVersion = 4;

    public static string Encode(PureRunSaveSnapshot snapshot) => ReplaceSchema(RunSaveDocumentV3.Encode(snapshot), SchemaVersion) + "\n";

    public static RunSaveDecodeResultV4 Decode(string json)
    {
        ...
        try
        {
            ...
            RunSaveDecodeResultV3 decoded = RunSaveDocumentV3.Decode(version == SchemaVersion ? ReplaceSchema(json, 3) : json);
