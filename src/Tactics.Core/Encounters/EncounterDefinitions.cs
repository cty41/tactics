...
public sealed record BattleLayoutDefinition(ContentId ContentId, IReadOnlyList<GridPoint> PartySpawns,
    IReadOnlyList<GridPoint> EnemySpawns, IReadOnlyList<GridPoint> BlockedCells,
    IReadOnlyList<GridPoint>? ShallowWaterCells = null)
{
    public IReadOnlyList<GridPoint> ShallowWater => ShallowWaterCells ?? Array.Empty<GridPoint>();
}
...
public sealed class EncounterResolver
{
    public ResolvedEncounter Resolve(EncounterDefinition encounter, BattleLayoutDefinition layout)
    {
        ...
        if(layout.ShallowWater.Any(cell=>cell.X<0||cell.X>=10||cell.Y<0||cell.Y>=10)||layout.ShallowWater.Distinct().Count()!=layout.ShallowWater.Count) throw new ArgumentException("Layout contains invalid shallow-water cells.");
