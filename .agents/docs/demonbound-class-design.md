...
厄运魔刃是四方向主动近战魔法技能。选择相邻方向格后，角色挥剑释放一道紫色半月斩，依次穿过前方第 1、2 格；它忽略墙体和单位阻挡，只伤害当前控制关系下的敌对单位。两格内没有合法目标时不能施放，失败或取消不消耗 MP、腐化或 RNG。三个等级均消耗 3 MP、产生 3 腐化，每个目标独立判定暴击。

| 等级 | 效果 |
|---|---|
| Lv1 | 造成 `5 + max(0, 当前魅力 - 5)` 点魔法伤害，不附加状态。 |
| Lv2 | 造成 `6 + max(0, 当前魅力 - 5)` 点魔法伤害；成功命中且目标存活时施加厄运，使其直接伤害输出降低 25%，持续目标的 1 个回合。 |
| Lv3 | 造成 `7 + max(0, 当前魅力 - 5)` 点魔法伤害；厄运持续时间提高至目标的 2 个回合。 |

目标按离施法者由近到远结算；同一距离以稳定 UnitId 排序。厄运不叠加，只刷新持续时间。基础攻击和横扫均不获得厄运魔刃增伤，也不施加、刷新或触发厄运。
