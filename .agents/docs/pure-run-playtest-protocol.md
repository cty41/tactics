...
现有 Gameplay Test Framework、Core/Application/Godot 测试适合证明确定性配方、路线事务、真实输入链、结算/恢复规则、RunSummary 和功能回归；它们不能替代玩家对策略意义、信息清晰度、疲劳、挫败原因和记录负担的判断。

人工试玩也不替代自动化逻辑断言。发现可稳定复现的功能错误时，应单独记录复现步骤，后续补自动化；本 P0-C 只建立协议与空 CSV 模板，不修改测试、C# 或 Godot Resource。
