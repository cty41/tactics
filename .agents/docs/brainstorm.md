...
##玩法
- 考虑可以让玩家接入llm byok来制作自己的游戏角色内容:可以直接用我们目前已经实现的文字需求变为实现的工具

## 职业想法
- 半人半恶魔职业已收束为[魔剑士职业设计](demonbound-class-design.md)；非大师内容已进入实现，正式美术、完整 VFX/音频和人工验收仍待后续收口。
- 加一个全能类型,没有主属性,类似dota2类的全能英雄,数值计算为单独考虑下

## 表现想法

- 开宝箱时使用类似《暗黑破坏神》的金币和装备弹出效果。
- 事件属性检定未来可增加类似《博德之门 3》的投骰演出，展示检定角色、最终成功率、骰子结果以及成功或失败结算。-不是个好点子,暂停
- 加战斗内的气泡文字聊天
-是否考虑最小成本的前提下丰富事件,休息,商店的表现,也在tile地图上,有单位,有特殊交互物(火堆,商人,npc,甚至遭遇战),事件不同选项可以触发场景变化(变为战斗).
