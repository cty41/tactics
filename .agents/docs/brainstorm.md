...
- 篝火除了休息还能做饭?
- 装备系统,多个类型的武器,武器和基础技能绑定,但职业技能绑武器类型(亚马逊的标枪类技能只能使用标枪,但法师的技能不限制武器,只是武器的加成可能用不到).这里复杂了.还是维持简化的职业有限定的武器和副手.避免和美术表现有冲突

## 职业想法
- 半人半恶魔职业已收束为[魔剑士职业设计](demonbound-class-design.md)；非大师内容已进入实现，正式美术、完整 VFX/音频和人工验收仍待后续收口。done
- 加一个全能类型,没有主属性,类似dota2类的全能英雄,数值计算为单独考虑下 done
...
- 开场选择 2 名角色；每通过一张地图，可在新角色、珍贵装备或其他等价奖励之间选择。不做,暂时还是n选3冒险
...
- 本节只记录 Unity → Godot 迁移中可能具有公共价值的技术缺口，不代表决定开源、确定仓库结构或承诺维护周期。 doing
- 开源工作不能成为迁移前置条件；优先完成 Tactics 的真实迁移验证，再从至少两个实际领域中提炼稳定公共接口。 doing
