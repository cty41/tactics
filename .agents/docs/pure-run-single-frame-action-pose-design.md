...
赤柴基础试玩切片已在真实战斗确认切图时机、投掷复用与跨技能施法姿态成立。当前进一步接入已批准的无矛受击方向对：`HitFamily` 在恢复段开始退出，`Default / Unarmed` 共用同一对 Sprite，姿态期间不改变 `AmazonBattleState.IsSpearHeld`。羊魔动作图继续延期，必须先完成人工真实战斗受击 QA。
