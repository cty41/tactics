...
timestamp: "2026-08-15T12:37:16+08:00"
status: active
...
source_fingerprint: sha256:2b0281f4aaaf84934fbf763006cdf144260f03023ff110c1e7549d8adc840005
---
...
Godot 正式 UI 收口采用原生 `Theme`/`Control` 等价表达 Unity UI Toolkit 的项目视觉语言，不复制 UXML/USS。根 Theme 统一近黑背景、半透明面板、橙色强调边、白灰文本以及 Button 的 normal/hover/pressed/focus/disabled 状态，并通过语义 variation 区分主按钮、紧凑控制、战斗动作和卡片。Home 使用居中的 TACTICS/PURE RUN 主菜单面板并恢复 Options 入口；普通 Run 页面共享标题面板，Esc Pause 使用最高层中央菜单而不再把文字直接叠在战场单位上。Battle HUD 将单位详情、Round/Turn、播放控制、横向动作栏和 End Turn 放入五个固定安全区面板，面板不参与鼠标命中且绘制在棋盘 Actor 之上，生产输入节点与 Gameplay Spec 语义保持不变。Map、Progression、Inventory、Settlement 与 Summary 继续消费该共享视觉合同；UI Theme 不进入 gameplay Catalog，canonical Catalog 保持 131。
