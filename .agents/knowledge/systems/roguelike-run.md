...
timestamp: "2026-08-24T23:09:20+08:00"
status: active
...
source_fingerprint: sha256:729ffdfb4a5afe5ce06de79c151fd62fb7325a0c5b2efa77583a771e9522cca0
---
...
Godot Adventure Board 使用正式 Tile 投影和生产输入链呈现出口、队伍位置与节点状态。共享 `GodotIsometricTileMapSurface` 提供 10×10、96×48 的 Terrain/Decoration/Mask/Overlay TileMapLayer 与确定性拾取。应用启动不再经过 Home：可恢复状态直接 Continue，无有效存档直接创建 PendingRunSetup 并进入开始营地。营地选择按点击顺序逐人持久化，第一名为可移动领队，选择不可撤销；专用地图输入层以可见 Sprite、边界容错和前景 ZIndex 解析候选，三人满员才解锁出口。Start 地图集使用 Current/Overview 双状态：Current 完整框住真实 Start 并允许选人/移动；Overview 采用固定可读缩放，仅允许 Preview、右键与键盘平移，不提供滚轮缩放。`M` 全局切换双态，`F/Home` 返回领队。每个节点的标题、类型与层数锚定在自身 10×10 TileMap 上方中央；模式提示与固定 UI 独立于地图变换。战斗节点共享表面仍由后续切片处理。
